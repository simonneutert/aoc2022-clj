...
(defn move-stack [command stack]
  (let [[n f t] command]
    (loop [i (first (range n))
           xstack stack]
      (cond
        (>= i n) xstack
        :else (let [from-column (into [] (remove empty? (nth xstack (dec f))))
                    to-columns (into [] (remove empty? (nth xstack (dec t))))
                    item (first from-column)
                    new-to-column (into [] (cons item to-columns))
                    new-from-column (into [] (drop 1 from-column))
                    next-stack (-> (assoc xstack (dec f) new-from-column)
